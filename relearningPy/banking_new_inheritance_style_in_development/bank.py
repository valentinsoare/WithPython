...
def using_arguments_in_debugging(to_file: bool = False):
    def info(self):
        given_list_with_result: list = []
        given_list_with_result.extend([f"Time: {datetime.now(timezone.utc)}",
                                       f"Class: {self.__class__.__name__}",
                                       f"id: {hex(id(self))}"])

        for i, j in vars(self).items():
            given_list_with_result.append(f"{i}: {j}")

        if to_file:
            with open(file='debug.txt', mode='a+', encoding='utf-8') as debug_file:
                print(f"{given_list_with_result}", file=debug_file)

        return given_list_with_result
    return info


def debug_mode(to_file):
    def debug_info(cls):
        cls.debug = using_arguments_in_debugging(to_file)
        return cls
    return debug_info


@debug_mode(to_file=True)
class Bank:
    ...
    def open_savings_account(self, owner, balance, currency, interest_rate, owner_address) -> SavingsAccount:

        """Open savings account using arguments from when the function is called"""
    ...
    def open_checking_account(self, owner, balance, currency, transaction_fee, owner_address) -> CheckingAccount:

        """Open checking account using the arguments in the function and then _open_account is
        called in ordr to do so."""
    ...
    def open_salary_account(self, owner, balance, currency, transaction_salary_fees, type_of_commission, commission_amount,
                            credit_card_withdraw_fees, annual_maintenance_fees, owner_address) -> SalaryAccount:

        """Open salary account function called _open_account"""

        return self._open_account(owner=owner, balance=balance, currency=currency, transaction_salary_fees=transaction_salary_fees,
                                  ...
                                  owner_address=owner_address, account_type='salary_account')
    ...
    def _open_account(self, *, account_type: str, owner: str, balance: Decimal, currency: str, owner_address: str, interest_rate: Decimal = Decimal('0.00'),
                      ...
                      commission_amount: Decimal = Decimal('0.00'), credit_card_withdraw_fees: Decimal = Decimal('0.00'),
                      annual_maintenance_fees = Decimal('0.00')) -> Union[SavingsAccount, CheckingAccount, SalaryAccount]:

        """Open account and take into consideration type_of_account variable.
        This is called by account type functions."""
    ...
    def search_credits_by_owner(self, *, number_account: str = None, owner_account: str = None, account_type: str = None,
        ...
        """Search credits using variables specific to the owner"""

        credits_to_returned: dict = dict()
    ...
    def search_account(self, *, account_number=None, owner=None, type_of_account=None, account_currency=None) -> Union[dict, None]:
        ...
        """Accounts by variables specific to owner and account. These are not integers or float variables."""
    ...
    def search_account_numerical_values(self, *, initial_balance=None, interest_rate=None, transaction_fee=None) -> Union[dict, None]:
        ...
        """Searching account by variables with numeric values."""
    ...
    def create_credit_card(self, account_number: str, owner: str, balance: Decimal, currency: str, credit_card_withdraw_fees: Decimal,
                           annual_maintenance_fees: Decimal, card_type: str, type_of_commissions: str, commission_amount: Decimal,
                           transaction_fees: Decimal, serial_number: str, expiration_date: str, **kwargs) -> CreditCard:
    ...
    def search_credits(self, *, credit_amount=None, period_of_credit=None, interest_rate=None, bank_profit=None,
        ...
        """Search credits using comparison operators, <, >, <= and so on."""
        ...
        number_of_arguments_given: int = len(arguments_we_have)
    ...
    def __bool__(self):
        ...
        """Boolean value of bank object if there is at least one open account in the bank."""

        return True if len(self._accounts) == 0 else False
...
def _generate_new_account_number(*, name_of_bank: str, country_of_bank: str, city_of_bank: str) -> str:
    ...
    """Generate new unique account number when an account is open
    taking into consideration the name of the bank, country and city."""
...
def check_str_input(*, given_value: str, type_of_input: str) -> None:
    ...
    """Check variable type if is a string containing letters."""
